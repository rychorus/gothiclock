(function () {
  const { CENTER_INDEX, cloneOffsets } = window.GothicLockpickCore;

  function getActionDelta(normalizedLink, delta) {
    return normalizedLink.map((value) => value * delta);
  }

  function groupConsecutiveKeys(keys) {
    const groups = [];
  ...
  function formatKeyGroups(groups) {
    return groups
      .map(({ key, count }) => (count > 1 ? `${key}\u00d7${count}` : key))
      .join(" ");
  }

  function buildSolutionChunks(moves, startOffsets, links) {
    const chunks = [
      {
        id: "reset",
        label: "R",
  ...
  function buildSolutionPlan({ plateCount, links }, startOffsets) {
  ...
  function buildSolutionCommandString(chunks) {
    if (!Array.isArray(chunks) || !chunks.length) {
      return "";
    }

    return chunks.flatMap((chunk) => chunk.keys).join("");
  }

  function buildWasdSequence(chunks) {
    if (!Array.isArray(chunks) || !chunks.length) {
      return [];
    }
